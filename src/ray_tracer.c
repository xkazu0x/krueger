...
internal b32
write_bmp(const char *filename, u32 width, u32 height, u32 compression, const void *data) {
    b32 result = false;
    u32 image_size = width*height*sizeof(u32);
    BMP_Header header = {
        .file_type = 0x4D42,
        .file_size = sizeof(BMP_Header) + image_size,
        .reserved1 = 0,
        .reserved2 = 0,
        .data_offset = sizeof(BMP_Header),
...
main(void) {
    ...
    if (write_bmp(filename, image.width, image.height, 0, image.pixels)) {
        printf("\n[INFO]: output: %s\n", filename);
    } else {
        fprintf(stderr, "[ERROR]: Failed to write bmp: %s\n", filename);
    }
    return(0);
}
