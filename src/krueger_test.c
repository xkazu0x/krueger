...
internal void
draw_rectf(Image image,
           f32 x0, f32 y0,
           f32 x1, f32 y1,
           Vector3 color) {
  if (x0 > x1) swap_t(f32, x0, x1);
  if (y0 > y1) swap_t(f32, y0, y1);
  s32 min_x = clamp_bot(0, round_t(s32, x0));
  s32 min_y = clamp_bot(0, round_t(s32, y0));
  s32 max_x = clamp_top(round_t(s32, x1), (s32)image.width);
  s32 max_y = clamp_top(round_t(s32, y1), (s32)image.height);
  u32 color32 = color32_from_vector3(color);
  for (s32 y = min_y; y < max_y; ++y) {
    for (s32 x = min_x; x < max_x; ++x) {
      image.pixels[y*image.pitch + x] = color32;
...
draw_rectv(Image image, Vector2 min, Vector2 max, Vector3 color) {
  draw_rectf(image, min.x, min.y, max.x, max.y, color);
}

internal void
draw_texture(Image dst, Image src, s32 x, s32 y, Vector3 color) {
  ...
  for (s32 dy = min_y; dy < max_y; ++dy) {
    ...
    for (s32 dx = min_x; dx < max_x; ++dx) {
      s32 sx = dx - min_x;
      u32 src_color = src.pixels[sy*src.pitch + sx];
      color.r = clamp(0.0f, color.r, 1.0f);
      color.g = clamp(0.0f, color.g, 1.0f);
      color.b = clamp(0.0f, color.b, 1.0f);
      u8 r = round_t(u8, mask_red(src_color)*color.r);
      u8 g = round_t(u8, mask_green(src_color)*color.g);
      u8 b = round_t(u8, mask_blue(src_color)*color.b);
      u8 a = 0xFF;
      dst.pixels[dy*dst.pitch + dx] = pack_rgba(r, g, b, a);
    }
  }
}

internal void
draw_text(Image image, Font font, char *text, s32 x, s32 y, Vector3 color) {
  s32 gw = font.glyph_w;
  s32 gh = font.glyph_h;
  s32 gx = x;
  s32 gy = y;
  ...
  for (uxx i = 0; i < text_len; ++i) {
    ...
    draw_texture(image, glyph, gx, gy, color);
    gx += gw;
  }
}
...
internal void
draw_debug_info(Game_State *state, Image draw_buffer, Clock *time, Font font) {
  s32 x = 0, y = 0;
  Vector3 color = make_vector3(0.6f, 0.15f, 0.1f);

  f32 fps = million(1.0f)/time->dt_us;
  f32 ms = time->dt_us/thousand(1.0f);

  local char fps_str[256];
  sprintf(fps_str, "%.2fFPS", fps);
  x = draw_buffer.width - (s32)cstr_len(fps_str)*font.glyph_w;
  draw_text(draw_buffer, font, fps_str, x, y, color);

  local char ms_str[256];
  sprintf(ms_str, "%.2fMS", ms);
  x = draw_buffer.width - ((s32)cstr_len(ms_str)+1)*font.glyph_w;
  y += font.glyph_h;
  draw_text(draw_buffer, font, ms_str, x, y, color);

  local char msc_str[256];
  sprintf(msc_str, "%.2fMSC", state->ms);
  x = draw_buffer.width - (s32)cstr_len(msc_str)*font.glyph_w;
  y += font.glyph_h;
  draw_text(draw_buffer, font, msc_str, x, y, color);
}
...
KRUEGER_INIT_PROC(krueger_init) {
  ...
  char *chars = "ABCDEFGHIJKLMNOPQRSTUVWXYZ "
                "0123456789.,!?'\"-+=/\\%()<> ";
  u32 num_chars_max = (u32)cstr_len(chars);
  u32 num_chars_x = num_chars_max/2;
  u32 num_chars_y = num_chars_max/num_chars_x;

  state->font_image = load_bmp("../res/font.bmp", &state->perm_arena, &state->temp_arena);
  state->font = make_font(chars, num_chars_x, num_chars_y, state->font_image, 8, 8);

  state->draw_debug_info = true;
  state->tile_size = 8;

  state->player.size = make_vector2((f32)state->tile_size, (f32)state->tile_size);
  state->player.pos = make_vector2(back_buffer->width/4.0f, back_buffer->height/2.0f);
  state->player.speed = state->tile_size*8.0f;

  state->max_pipe_count = 4;
  state->pipes = arena_push_array(&state->perm_arena, Entity, state->max_pipe_count);
}
...
KRUEGER_FRAME_PROC(krueger_frame) {
  ...
  state->ms += time->dt_us/1000.0f;

  Image draw_buffer = *back_buffer;
  draw_clearv(draw_buffer, make_vector3(0.05f, 0.05f, 0.05f));
  s32 tile_count_x = draw_buffer.width/state->tile_size;
  s32 tile_count_y = draw_buffer.height/state->tile_size;
  for (s32 y = 0; y < tile_count_y; ++y) {
    s32 y0 = y*state->tile_size;
    s32 y1 = y0 + state->tile_size;
    for (s32 x = 0; x < tile_count_x; ++x) {
      s32 x0 = x*state->tile_size;
      s32 x1 = x0 + state->tile_size;
      Vector3 color = make_vector3(0.1f, 0.1f, 0.1f);
      if ((x + y)%2 == 0) {
        color = make_vector3(0.2f, 0.2f, 0.2f);
      }
      draw_rect(draw_buffer, x0, y0, x1, y1, color);
    }
  }

  {   
    Entity *player = &state->player;
    f32 gravity = 1.6f*time->dt;

    if (kbd[KEY_SPACE].pressed) {
      player->vel.y = -player->speed*time->dt;
    }

    player->vel.y += gravity;
    player->pos.y += player->vel.y;

    if (player->pos.y > draw_buffer.height) {
      player->pos.y = (f32)draw_buffer.height;
      player->vel.y = 0.0f;
    }
    ...
    Vector2 half_size = vector2_div(player->size, 2);
    Vector2 min = vector2_sub(player->pos, half_size);
    Vector2 max = vector2_add(player->pos, half_size);
    Vector3 color = make_vector3(0.6f, 0.15f, 0.1f);
    draw_rectv(draw_buffer, min, max, color);
  }
  
  if (state->ms > 1500.0f) {
    if (state->pipe_count < state->max_pipe_count) {
      Entity *pipe = state->pipes + state->pipe_count++;
      pipe->size = make_vector2(state->tile_size*2.0f, draw_buffer.height/2.0f);
      pipe->pos = make_vector2((f32)draw_buffer.width, draw_buffer.height/2.0f);
      pipe->speed = state->tile_size*4.0f;
    }
    state->ms = 0.0f;
  }

  {
    for (u32 i = 0; i < state->pipe_count; ++i) {
      Entity *pipe = state->pipes + i;

      if (state->ms == 0.0f && pipe->pos.x < -pipe->size.x) {
        pipe->pos.x = (f32)draw_buffer.width;
      }
      pipe->pos.x -= pipe->speed*time->dt;

      Vector2 min = pipe->pos;
      Vector2 max = vector2_add(min, pipe->size);
      Vector3 color = make_vector3(0.6f, 0.6f, 0.6f);
      draw_rectv(draw_buffer, min, max, color);
    }
  }
  ...
  if (state->draw_debug_info) {
    draw_debug_info(state, draw_buffer, time, state->font);
  }
}
