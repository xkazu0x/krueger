...
internal _Win32_Window *
_win32_window_from_hwnd(HWND hwnd) {
  _Win32_Window *result = 0;
  for (each_node(_Win32_Window, window, _win32_gfx_state->first_window)) {
    if (window->hwnd == hwnd) {
      result = window;
      break;
    }
...
internal _Win32_Window *
_win32_window_alloc(void) {
  _Win32_Window *result = _win32_gfx_state->free_window;
  if (result) {
    stack_pop(_win32_gfx_state->free_window);
  } else {
    result = push_array(_win32_gfx_state->arena, _Win32_Window, 1);
  }
  mem_zero_struct(result);
  dll_push_back(_win32_gfx_state->first_window,
                _win32_gfx_state->last_window,
                result);
...
internal void
_win32_window_release(_Win32_Window *window) {
  ReleaseDC(window->hwnd, window->hdc);
  DestroyWindow(window->hwnd);
  dll_remove(_win32_gfx_state->first_window,
             _win32_gfx_state->last_window,
             window);
  stack_push(_win32_gfx_state->free_window, window);
}

internal Platform_Event *
_win32_push_event(Platform_Event_Type type, _Win32_Window *window) {
...
_win32_window_proc(HWND hwnd, UINT message, WPARAM wparam, LPARAM lparam) {
  LRESULT result = 0;
  if (_win32_event_arena) {
    _Win32_Window *window = _win32_window_from_hwnd(hwnd);
    switch (message) {
      case WM_CLOSE: {
        _win32_push_event(PLATFORM_EVENT_WINDOW_CLOSE, window);
      } break;
      case WM_SYSKEYDOWN:
      case WM_SYSKEYUP: {
        if (wparam == VK_F4) {
      ...
      case WM_KEYUP: {
        ...
        Platform_Event *event = _win32_push_event(type, window);
...
platform_graphics_init(void) {
  ...
  _win32_gfx_state->instance = GetModuleHandleW(0);

  WNDCLASSEXW window_class = {.cbSize = sizeof(window_class)};
  window_class.style = CS_VREDRAW | CS_HREDRAW;
  window_class.lpfnWndProc = _win32_window_proc;
  window_class.hInstance = _win32_gfx_state->instance;
  ...
  _win32_gfx_state->atom = RegisterClassExW(&window_class);
...
platform_window_open(String8 name, s32 width, s32 height) {
  ...
  HWND hwnd = CreateWindowExW(window_style_ex, MAKEINTATOM(_win32_gfx_state->atom),
                              window_name.str, window_style,
                              window_x, window_y, window_w, window_h,
                              0, 0, _win32_gfx_state->instance, 0);

  _Win32_Window *window = _win32_window_alloc();
  window->hwnd = hwnd;
