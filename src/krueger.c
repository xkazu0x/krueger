...
internal Mesh
load_obj(char *filepath, Arena *perm_arena, Arena *temp_arena) {
  Mesh mesh = {0};
  u64 file_size;
  u8 *file_data;
  Temp temp = temp_begin(temp_arena);
  if (read_entire_file(temp.arena, filepath, &file_data, &file_size)) {
    ...
    mesh.vertices = arena_push_array(perm_arena, Vector3, max_vertex_count);
    mesh.vertex_indices = arena_push_array(perm_arena, u32, max_vertex_index_count);
...
KRUEGER_INIT_PROC(krueger_init) {
  assert(sizeof(Krueger_State) <= memory->memory_size);
  Krueger_State *state = (Krueger_State *)memory->memory_ptr;

  uxx half_memory_size = memory->memory_size/2;
  uxx perm_memory_size = half_memory_size - sizeof(Krueger_State);
  uxx temp_memory_size = half_memory_size;

  u8 *perm_memory_ptr = (u8 *)memory->memory_ptr + sizeof(Krueger_State);
  ...
  state->mesh = load_obj("../res/monkey.obj", &state->perm_arena, &state->temp_arena);
